...
constexpr int AlignmentA =
    128 / cutlass::sizeof_bits<ElementA>::value;  // Alignment of A matrix in units of elements (up
                                                  // to 16 bytes)

// B matrix configuration
using LayoutB = cutlass::layout::ColumnMajor;  // Layout type for B matrix operand
constexpr int AlignmentB =
    128 / cutlass::sizeof_bits<ElementB>::value;  // Alignment of B matrix in units of elements (up
                                                  // to 16 bytes)
...
void cutlass_fp8_moe_gemm(ElementA* x, ElementB* weight, int64_t* total_rows_before_expert,
  ...
  Gemm::EpilogueOutputOp::Params epilogue_params{ElementAccumulator(1.0f),
...
TVM_REGISTER_GLOBAL("cutlass.moe_gemm_f8f8")
