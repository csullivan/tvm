...
def run_test(
    ...
    f_cublas = tvm.build(s, [A_cublas, B_cublas, C_cublas], target="cuda")
    a_cublas = tvm.nd.array(a_np, dev)
    b_cublas = tvm.nd.array(b_np, dev)
    c_cublas = tvm.nd.array(np.zeros((M, N), dtype=out_dtype), dev)
    cublas_closure = lambda: f_cublas.time_evaluator(f_cublas.entry_name, dev, number=500)(
        a_cublas, b_cublas, c_cublas
    )
    ...
    return lambda: f.time_evaluator(f.entry_name, dev, number=500)(a, b, c), cublas_closure
...
def test_f16f16f16_m16n16k16():
    ...
    if measure_perf and cublas_timer:
        print("cublas: %f GFLOPS" % (gflops / (cublas_timer().mean)))
